/**
 * Kata ulr - https://www.codewars.com/kata/factorial
 * Description:
 * In mathematics, the factorial of a non-negative integer n, denoted by n!, is the product of all positive integers less than or equal to n. For example: 5! = 5 * 4 * 3 * 2 * 1
 * = 120. By convention the value of 0! is 1.
 *
...
function factorial(n) {
    ...
    if (n < 0 || n > 12) {
        throw new RangeError('Provided digit is out of range');
    }

    return Array.from(Array(n).keys())
        .map(digit => digit + 1)
        .reduce((accumValue, currValue) => accumValue * currValue, 1);
}
